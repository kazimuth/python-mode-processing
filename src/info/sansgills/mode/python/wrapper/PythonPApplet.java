...
/**
 * 
 * An implementation of PApplet that contains a dictionary of Python functions
 * that it calls in lieu of implementing draw(), setup(), etc.
 * 
 * A instance of this is created and has all of its methods injected into the
 * python namespace. Then, the functions defined in the python code are sent
 * back to here, and this runs them in lieu of draw(), setup(), etc.
 * 
 * Note that setup() is always defined in Python Mode due to preprocessing.
 * 
 * Also, when this exits, it doesn't necessarily kill its process; see
 * ProcessingJythonWrapper for more details.
 * 
 */
